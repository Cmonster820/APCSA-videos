...
class uSubTest(Scene):
    def construct(self):
        ...
        self.remove(upplim)
        self.wait(0.5)
        explanation3p4 = MathTex(r"\frac{1}{5}u^{5}\biggr\rvert_{3}^{-6}=",r"\frac{8019}{5}")
        explanation3p4.next_to(du,DOWN)
        self.play(Write(explanation3p4[0]))
        box1 = SurroundingRectangle(explanation3p4[1],buff=0.1)
        box2 = SurroundingRectangle(questionfull[1],buff=0.1)
        copy = explanation3p4[1].copy()
        self.play(LaggedStart(Write(explanation3p4[1]),Create(box1),Transform(copy,questionfull[1]),Create(box2),lag_ratio=0.75))
        self.add(questionfull[1])
        self.remove(copy)
        self.wait(2)
        self.play(Uncreate(box1),Uncreate(box2))
        self.play(Unwrite(explanation3p4))
        self.play(Unwrite(du))
        self.play(Unwrite(explanation3p3))
        self.wait(2)
        self.play(Unwrite(questionfull))
        self.wait(2)
        questionfull = MathTex(r"\int_{0}^{3}\!\frac{1}{(3x+1)^{2}}~dx=",r"\frac{3}{10}")
        questionfull.to_edge(UP)
        question = questionfull[0].copy()
        question.move_to(ORIGIN)
        self.play(Write(question))
        ...
        self.remove(question)
        self.wait(2)
        explanation4p1 = MathTex(r"\int_{0}^{3}\!\frac{1}{",r"(3x+1)",r"^{2}}~dx")
        explanation4p1.next_to(questionfull,DOWN)
        self.play(Write(explanation4p1))
        self.wait(0.5)
        brace = Brace(explanation4p1[1])
        u = MathTex(r"u")
        u.next_to(brace,DOWN)
        self.play(LaggedStart(Write(brace),Write(u),lag_ratio=0.5))
        explanation4p2 = MathTex(r"\int_{x=0}^{x=3}\!\frac{1}{",r"(3x+1)",r"^{2}}~dx")
        explanation4p2.move_to(explanation4p1)
        newbrace = Brace(explanation4p2[1])
        newu = u.copy()
        newu.next_to(newbrace,DOWN)
        self.wait(0.5)
        self.play(Transform(explanation4p1,explanation4p2),Transform(u,newu),Transform(brace,newbrace))
        self.add(explanation4p2)
        self.remove(explanation4p1)
        self.wait(0.5)
        copy1 = explanation4p2[1].copy()
        copy2 = copy1.copy()
        upplim = MathTex(r"3(3)+1=10")
        lowlim = MathTex(r"3(0)+1=1")
        upplim.next_to(explanation4p2,RIGHT)
        lowlim.next_to(upplim,DOWN)
        self.play(LaggedStart(Transform(copy1,upplim),Transform(copy2,lowlim),lag_ratio=0.5))
        self.add(upplim)
        self.add(lowlim)
        ...
        self.play(Write(du))
        self.wait(1)
        newdu = MathTex(r"\frac{1}{3}",r"du=dx")
        newdu.next_to(u,DOWN)
        self.play(Transform(du,newdu))
        self.wait(0.5)
        copy = newdu[0].copy()
        explanation4p3 = MathTex(r"\frac{1}{3}",r"\int_",r"{u=1}^",r"{u=10}",r"\!u^{-2}~du")
        explanation4p3.move_to(explanation4p2)
        self.play(Transform(explanation4p2,explanation4p3),Unwrite(u),Unwrite(brace),Unwrite(du[1]),Transform(du[0],explanation4p3[0]),Transform(upplim,explanation4p3[3]),Transform(lowlim,explanation4p3[2]))
        self.add(explanation4p3)
        self.remove(explanation4p2)
        self.remove(du)
        self.remove(du[0])
        self.remove(upplim)
        self.remove(lowlim)
        self.wait(0.5)
        explanation4p4 = MathTex(r"\frac{1}{3}\left(-u^{-1}\biggr\rvert_{1}^{10}\right)")
        explanation4p4.move_to(explanation4p3)
        self.play(Transform(explanation4p3,explanation4p4))
        self.add(explanation4p4)
        self.remove(explanation4p3)
        explanation4p5 = MathTex(r"\frac{1}{3}\left(\left(-\frac{1}{10}\right)-\left(-\frac{1}{1}\right)\right)")
        explanation4p5.next_to(explanation4p4,DOWN)
        self.wait(0.5)
        self.play(Write(explanation4p5))
        self.wait(2)
        answer = MathTex(r"\frac{3}{10}")
        answer.move_to(explanation4p5)
        self.play(Transform(explanation4p5,answer))
        self.wait(0.5)
        box1 = SurroundingRectangle(answer)
        box2 = SurroundingRectangle(questionfull[1])
        copy = answer.copy()
        self.play(LaggedStart(Transform(explanation4p5,answer),Create(box1),Transform(copy,questionfull[1]),Create(box2),lag_ratio=0.75))
        self.wait(2)
        self.play(Uncreate(box1),Uncreate(box2))
        self.play(Unwrite(explanation4p5))
        self.add(questionfull[1])
        self.remove(copy)
        self.play(Unwrite(explanation4p4))
        self.wait(2)
        self.play(Unwrite(questionfull))
        self.wait(2)