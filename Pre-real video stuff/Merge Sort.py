...
class MergeSort(Scene):
    def construct(self):
        ...
        subgroup1 = VGroup(*[item for item in table[:len(table)//4]])
        subgroup2 = VGroup(*[item for item in table[len(table)//4:len(table)//2]])
        subgroup3 = VGroup(*[item for item in table[len(table)//2:len(table)*3//4]])
        subgroup4 = VGroup(*[item for item in table[len(table)*3//4:]])
        self.play(subgroup1.animate.shift([-0.5,0,0]),subgroup4.animate.shift([0.5,0,0]),divider1.animate.shift([-0.25,0,0]),divider2.animate.shift([0.25,0,0]))
        self.wait(2)